...
const ExplorePage = () => {
    ...
    const { data: featredEvents, isLoading: isFetrEvetsLoading } = useConvexQuery(api.explore.getFeaturedEvents, { limit: 3 })
console.log(featredEvents);
    ...
    const handleViewLocalEvents = () => {
        const city = currentUser?.localtion?.city || "Gurgaon";
    ...
    const isLoading = isFetrEvetsLoading || loadingLocal || loadingPopular;
    if (isLoading) {
        return (
        ...
        <>
            <div className='pb-12 text-center'>
                ...
                <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
                    Explore featured events, find what&apos;s happening locally, or browse events across Pakistan.
                </p>
            </div>

            {/* Featured Carousel */}
            {
                featredEvents && featredEvents.length > 0 && (
                    <div className="mb-16">
                        ...
                        >
                            <CarouselContent>
                                {
                                    featredEvents.map((event) => (
                                        <CarouselItem key={event._id}>
                                            <div onClick={() => handleEventClick(event.slug)} className='relative h-[400px] rounded-xl cursor-pointer overflow-hidden'>
                                                {
                                                    event.coverImage ? (
                                                ...
                                                <div className="relative flex h-full flex-col justify-end p-8 md:p-12">
                                                    ...
                                                    <h2 className="text-3xl md:text-5xl font-bold mb-3 text-whtite">
                                                        {event.title}
                                                    </h2>
                                                    <p className="text-lg mb-4 max-w-2xl line-clamp-2 text-white/90">
                                                        {event.description}
                                                    </p>
            ...
            {/* Popular Events across Country */}
            {
                popularEvents && popularEvents.length > 0 && (
                    <div className="mb-16">
                        <div className="mb-6">
                            <h2 className="text-3xl font-bold mb-1">Popular Across Pakistan</h2>
                            <p className="text-muted-foreground">Trendin events nationwide</p>
                        </div>
